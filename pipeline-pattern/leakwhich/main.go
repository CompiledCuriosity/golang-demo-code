...
// instrumented: each stage records the id it is about to send
var idsHolding, hashesHolding, hashed atomic.Int64
...
func hashes(in <-chan int) <-chan result {
	...
	go func() {
		defer close(out)
		for id := range in {
			res := result{id, hash(id)}
			hashed.Add(1)
			hashesHolding.Store(int64(id))
			out <- res
		}
	}()
	return out
...
func main() {
	...
	fmt.Println("hashes is holding out doc", hashesHolding.Load())
	fmt.Println("ids is holding out id", idsHolding.Load())
}
